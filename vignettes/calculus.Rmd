...
The result confirms the **Beta posterior**: $\text{Beta}(\alpha + k, \beta + n - k)$.

### Normal-Normal Conjugacy

For **Normal prior** with **Normal likelihood**, completing the square gives:
$$p(\mu | \bar{x}) \propto \exp(-A\mu^2 + B\mu)$$
...
When an integrand doesn't match any known kernel, an unevaluated `Integrate()` call is returned:
...
This allows symbolic expressions to remain unevaluated until the user decides how to proceed.
...
|----------|---------|-------------|
| `deriv.sym(expr, var)` | Symbolic differentiation | Treats `E()` as constant |
| `integrate.sym(expr, var, lower, upper)` | Definite integration | Recognizes Gamma, Gaussian, Beta kernels |

Both functions use **non-standard evaluation**—no `quote()` needed!

<div class="vignette-nav">
  <a href="derivations.html" class="nav-item prev">
    <span class="nav-label">Previous</span>
    <span class="nav-title">3. Derivations</span>
  </a>
  <a href="independence-latex.html" class="nav-item next">
