...
- `classify.type(sym)` - Inspects a symbol and returns `"number"`, `"rv"`, or `"const"` based on naming conventions (capitals = RV, lowercase = constant)
- `ensure.expression(expr)` - Handles NSE logic to normalize input expressions
...
- `expect.recursive(expr)` - Recursively transforms expressions by applying linearity of expectation
...
- `simplify.expr(expr)` - Recursive simplifier that prunes identity elements
